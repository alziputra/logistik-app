...
export default function BangunanSarana({
  ...
  const handleSave = async (e) => {
    ...
    try {
      if (editingId) {
        await updatePengamananKorporasi(editingId, formData);
      } else {
        await addPengamananKorporasi(formData);
      }
      setIsModalOpen(false);
      if (loadAllData) loadAllData();
    } catch (err) {
      console.error("Gagal menyimpan data sarana pengamanan:", err);
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const handleDelete = async (id) => {
    if (!window.confirm("Apakah Anda yakin ingin menghapus data pengamanan ini?")) return;
    try {
      await deletePengamananKorporasi(id);
      if (loadAllData) loadAllData();
    } catch (err) {
      console.error("Gagal menghapus data pengamanan:", err);
    }
  };
    ...
    <div className="max-w-7xl mx-auto p-6 animate-in fade-in duration-300">
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-6 gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400">Manajemen perangkat CCTV, kamera pengawasan, dan sistem pengamanan fisik unit kerja.</p>
          </div>
        </div>
      ...
      <div className="bg-slate-900 rounded-2xl border border-slate-800 shadow-xl overflow-hidden">
        <div className="p-4 border-b border-slate-800 flex flex-col sm:flex-row justify-between items-center gap-4">
          <div className="relative w-full sm:w-80">
            ...
            <input
              type="text"
              placeholder="Cari unit kerja, kode, vendor..."
              value={searchQuery}
          ...
          <span className="text-xs font-semibold text-slate-400">Total Unit: {filteredData.length}</span>
        </div>
          ...
          <table className="w-full text-left text-xs whitespace-nowrap">
            <thead className="bg-slate-950 text-slate-400 border-b border-slate-800 font-bold uppercase tracking-wider">
              <tr>
                ...
                <th className="px-4 py-3">Vendor Pengamanan</th>
                <th className="px-4 py-3 text-center">Jumlah Kamera</th>
                <th className="px-4 py-3">Aplikasi CCTV</th>
              ...
              {paginatedData.length === 0 ? (
                <tr>
                  <td colSpan={userRole === "admin" ? "8" : "7"} className="px-6 py-8 text-center text-slate-500 italic">
                    Belum ada data sarana pengamanan terdaftar.
                  </td>
                </tr>
              ) : (
                paginatedData.map((item, idx) => (
                  <tr key={item.id || idx} className="hover:bg-slate-800/50 transition-colors">
                    ...
                    <td className="px-4 py-3 font-bold text-slate-100">{item.nama_unit_kerja || "-"}</td>
                    <td className="px-4 py-3 font-mono text-emerald-400">{item.kode_unit_kerja || "-"}</td>
                    <td className="px-4 py-3 text-slate-300">{item.vendor || "-"}</td>
                    <td className="px-4 py-3 text-center font-bold text-slate-200">{item.jumlah_kamera || 0}</td>
                    <td className="px-4 py-3 text-slate-300">{item.nama_aplikasi || item.aplikasi || "-"}</td>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/75 z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl w-full max-w-lg flex flex-col max-h-[90vh] overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-800 flex justify-between items-center bg-slate-900 shrink-0">
              <h3 className="font-bold text-lg text-slate-100">{editingId ? "Edit Sarana Pengamanan" : "Tambah Sarana Pengamanan"}</h3>
            ...
            <form onSubmit={handleSave} className="p-6 overflow-y-auto custom-scrollbar space-y-4">
              <div>
                <label className="block text-xs font-semibold text-slate-300 mb-1">Nama Unit Kerja *</label>
                <input
                  required
                  value={formData.nama_unit_kerja || ""}
                  onChange={(e) => setFormData({ ...formData, nama_unit_kerja: e.target.value })}
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-300 mb-1">Kode Unit Kerja</label>
                <input
                  value={formData.kode_unit_kerja || ""}
                  onChange={(e) => setFormData({ ...formData, kode_unit_kerja: e.target.value })}
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-300 mb-1">Vendor Pengamanan</label>
                <input
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-300 mb-1">Jumlah Kamera CCTV</label>
                <input
                  type="number"
                  value={formData.jumlah_kamera || ""}
                  onChange={(e) => setFormData({ ...formData, jumlah_kamera: e.target.value })}
                  className="w-full px-3 py-2 bg-slate-800 border border-slate-700 rounded-xl text-xs text-slate-100 outline-none focus:border-emerald-500"
                />
              </div>
              <div>
                <label className="block text-xs font-semibold text-slate-300 mb-1">Nama Aplikasi</label>
                <input
                  value={formData.nama_aplikasi || ""}
                  onChange={(e) => setFormData({ ...formData, nama_aplikasi: e.target.value })}
                  className="w-full px-3 py-2 bg-slate-800 border border-slate-700 rounded-xl text-xs text-slate-100 outline-none focus:border-emerald-500"
                  placeholder="Hilook / Hikvision..."
                />
              </div>
              <div className="flex justify-end gap-3 pt-4 border-t border-slate-800">
                ...
                <button type="submit" disabled={isSaving} className="px-5 py-2 text-xs font-semibold text-white bg-emerald-600 hover:bg-emerald-500 rounded-xl flex items-center gap-2 cursor-pointer disabled:opacity-50">
                  ...
                  {editingId ? "Simpan Perubahan" : "Simpan Sarana"}
                </button>
              </div>
            </form>
          </div>
