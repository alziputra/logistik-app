  ...
  const onSubmit = async (e) => {
    ...
    if (editingInv) {
      ...
      try {
        ...
        const updatedInv = {
          nama: form.get("nama"), stok: Number(form.get("stok")) || 0, satuan: form.get("satuan") || "Pcs",
          ...
          masa_sewa_bulan: Number(form.get("masa_sewa_bulan")) || 0, status: form.get("status") || "Inventaris"
        };
        await updateDoc(doc(db, "artifacts", appId, "public", "data", "inventory", editingInv.id), updatedInv);
        setLocalInventory(prev => prev.map(item => item.id === editingInv.id ? { id: editingInv.id, ...updatedInv } : item));
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div className="p-6 border-b border-gray-100 bg-gray-50/50 flex flex-col lg:flex-row justify-between items-center gap-4">
          ...
          <div className="flex flex-wrap w-full lg:w-auto gap-3 items-center">
            ...
            <div className="bg-blue-50 text-blue-700 px-5 py-2.5 rounded-xl text-sm font-semibold shrink-0">Total: {filteredInventory.length}</div>
            {userRole === "admin" && (
              <button onClick={openAdd} className="bg-blue-600 hover:bg-blue-700 text-white px-5 py-2.5 rounded-xl text-sm font-medium shadow-sm transition-colors flex items-center gap-2 shrink-0">
          ...
          <table className="w-full text-left min-w-[1300px]">
            <thead>
              <tr className="border-b-2 text-gray-500 text-sm">
                ...
                <th className="pb-4"><CalendarDays className="w-4 h-4 inline mr-2"/> Mulai</th>
                <th className="pb-4"><CalendarDays className="w-4 h-4 inline mr-2"/> Selesai</th>
              ...
              {filteredInventory.map((inv, index) => {
                ...
                return (
                  <tr key={inv.id} className="border-b border-gray-50 hover:bg-gray-50/80">
                    ...
                    <td className="py-4 text-center"><span className={`px-3 py-1 rounded-md text-[11px] font-bold border ${getStatusBadge(statusVal)}`}>{statusVal}</span></td>
                    {userRole === "admin" && (
                      <td className="py-4 text-right">
                        <div className="flex justify-end gap-1.5">
                          <button onClick={() => openEdit(inv)} className="p-2 text-blue-600 hover:bg-blue-50 rounded-lg"><Edit className="w-4 h-4" /></button>
                          <button onClick={() => askDelete(inv)} className="p-2 text-red-600 hover:bg-red-50 rounded-lg"><Trash2 className="w-4 h-4" /></button>
                        </div>
                      </td>
                    )}
                  </tr>
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-4xl flex flex-col max-h-[85vh] animate-in zoom-in-95 duration-200 relative">
            <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50 shrink-0 rounded-t-2xl">
              <div className="flex items-center gap-3">
                <div className="bg-blue-100 p-2 rounded-xl">{editingInv ? <Edit className="w-5 h-5 text-blue-600" /> : <Database className="w-5 h-5 text-blue-600" />}</div>
                <h3 className="font-bold text-lg text-gray-800">{editingInv ? "Edit Data Barang" : "Tambah Master Barang"}</h3>
              </div>
            ...
            <div className="p-6 overflow-y-auto custom-scrollbar">
              <form id="formBarang" onSubmit={onSubmit} className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                  <div className="md:col-span-2"><label className="block text-sm mb-1.5">Nama Barang *</label><input name="nama" defaultValue={editingInv?.nama||""} required className="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl" /></div>
                  <div><label className="block text-sm mb-1.5">Stok</label><input name="stok" type="number" defaultValue={editingInv?.stok||0} min="0" className="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl" /></div>
                  <div><label className="block text-sm mb-1.5">Satuan</label><select name="satuan" defaultValue={editingInv?.satuan||"Pcs"} className="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl"><option>Pcs</option><option>Unit</option><option>Box</option><option>Set</option></select></div>
                  <div className="md:col-span-2"><label className="block text-sm mb-1.5">Nama Vendor</label><input name="vendor_nama" defaultValue={editingInv?.vendor_nama||""} className="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl" /></div>
                  <div><label className="block text-sm mb-1.5">No. SPK</label><input name="no_spk" defaultValue={editingInv?.no_spk||""} className="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl" /></div>
                  <div><label className="block text-sm mb-1.5">No. PKS</label><input name="no_pks" defaultValue={editingInv?.no_pks||""} className="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl" /></div>
                  <div><label className="block text-sm mb-1.5">Tgl Mulai</label><input name="tanggal_mulai" type="date" defaultValue={editingInv?.tanggal_mulai||""} onChange={calculateFormLogic} className="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl" /></div>
                  <div><label className="block text-sm mb-1.5">Tgl Selesai</label><input name="tanggal_selesai" type="date" defaultValue={editingInv?.tanggal_selesai||""} onChange={calculateFormLogic} className="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl" /></div>
                  <div><label className="block text-sm mb-1.5">Status</label><input type="hidden" name="status" value={calculatedStatus} /><input type="text" readOnly value={calculatedStatus} className="w-full px-4 py-2.5 bg-gray-100 border border-gray-200 rounded-xl text-gray-500" /></div>
                  <div><label className="block text-sm mb-1.5">Masa Sewa</label><input name="masa_sewa_bulan" type="number" min="0" defaultValue={editingInv?.masa_sewa_bulan||""} className="w-full px-4 py-2.5 bg-gray-100 border border-gray-200 rounded-xl text-gray-500" readOnly /></div>
                  <div className="md:col-span-4 flex justify-end gap-3 mt-4 border-t pt-6 shrink-0">
                    <button type="button" onClick={() => setIsModalOpen(false)} disabled={isSaving} className="px-6 py-2.5 text-gray-600 hover:bg-gray-100 rounded-xl transition-colors">Batal</button>
                    <button type="submit" disabled={isSaving} className="px-8 bg-blue-600 hover:bg-blue-700 text-white py-2.5 rounded-xl flex items-center gap-2">{isSaving ? <Loader2 className="w-5 h-5 animate-spin"/> : <Plus className="w-5 h-5"/>} {editingInv ? "Simpan Perubahan" : "Simpan Barang"}</button>
                  </div>
                </div>
              </form>
            </div>
        ...
        <div className="fixed inset-0 bg-black/60 z-[200] flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="p-6 text-center">
              ...
              <p className="text-sm text-gray-500">Yakin hapus <span className="font-bold text-gray-800">{deleteConfirm.name}</span>?</p>
            </div>
            <div className="flex border-t border-gray-100">
              <button onClick={() => setDeleteConfirm({show:false, id:null, name:""})} disabled={isSaving} className="flex-1 px-4 py-4 text-sm font-bold text-gray-500 hover:bg-gray-50 border-r border-gray-100">BATAL</button>
