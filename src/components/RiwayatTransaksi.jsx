...
export default function RiwayatTransaksi({
  ...
  const exportToExcel = () => {
    ...
    link.setAttribute(
      "download",
      `Riwayat_Transaksi_Logistik_${new Date().toISOString().split("T")[0]}.csv`,
    );
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 sm:p-6 animate-in fade-in duration-300">
      <h2 className="text-xl sm:text-2xl font-bold text-gray-800 mb-5 tracking-tight">
        Pusat Riwayat Transaksi
      </h2>
          ...
          <div className="flex flex-col sm:flex-row w-full lg:w-auto gap-2 items-center">
            <div className="relative w-full sm:w-64">
              ...
              <input
                ...
                className="w-full pl-9 pr-3 py-2 bg-white border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm shadow-sm transition-all"
              />
            </div>
            <button
              onClick={exportToExcel}
              disabled={filteredTransactions.length === 0}
              className="w-full sm:w-auto px-4 py-2 bg-green-600 hover:bg-green-700 disabled:bg-gray-300 text-white font-medium rounded-lg text-sm transition-colors flex items-center justify-center gap-2 shadow-sm whitespace-nowrap"
            >
          ...
          <table className="w-full text-left min-w-[900px] text-sm">
            <thead>
              <tr className="text-xs text-gray-500 border-b border-gray-100 bg-white uppercase tracking-wide">
                ...
                <th className="py-3 px-5 font-semibold min-w-[200px]">
                  Barang Terkait
                </th>
                <th className="py-3 px-5 font-semibold min-w-[200px]">
                  Pihak Terlibat
                </th>
                <th className="py-3 px-5 font-semibold text-right w-28">
                  Aksi
                </th>
              </tr>
            </thead>
                ...
                filteredTransactions.map((trx) => (
                  ...
                  >
                    ...
                    <td className="py-3 px-5">
                      <span
                        className={`inline-flex px-2 py-1 rounded text-[11px] font-bold tracking-wide border ${trx.jenisTransaksi === "Barang Masuk" ? "bg-green-50 text-green-700 border-green-100" : "bg-orange-50 text-orange-700 border-orange-100"}`}
                      >
                        {trx.jenisTransaksi}
                      </span>
                    </td>
                    <td className="py-3 px-5 text-gray-700">
                      {trx.items && trx.items.length > 0 ? (
                        <div
                          ...
                          title={trx.items.map((i) => i.nama).join(", ")}
                        >
                          {trx.items.map((i, idx) => (
                            <span key={i.id || idx}>
                              {i.nama}{" "}
                    ...
                    <td className="py-3 px-5 text-xs">
                      <p className="text-gray-800 font-semibold truncate max-w-[200px]">
                        {trx.pengirimNama || "?"}
                      </p>
                      <p className="text-gray-500 flex items-center gap-1 mt-0.5 truncate max-w-[200px]">
                        <ArrowLeft className="w-3 h-3 transform rotate-180 shrink-0" />
                        {trx.penerimaNama || "?"}
                      </p>
                    </td>
                    <td className="py-3 px-5 text-right">
                      <button
                        ...
                        className="text-xs text-blue-600 hover:text-blue-800 font-bold bg-blue-50 hover:bg-blue-100 px-3 py-1.5 rounded transition-colors inline-block whitespace-nowrap"
                      >
                        Detail
                      </button>
                    </td>
